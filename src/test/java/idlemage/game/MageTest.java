...
public class MageTest {
	...
	public void test() {
		...
		assertEquals(0D, mage.getManaIncome(), 0.0001);
		...
		assertEquals(STARTING_BUILDING.income(0), mage.getManaIncome(), 0.0001);

		mage.buy(STARTING_BUILDING.getName());
		mana -= STARTING_BUILDING.buildCost(1);
		assertEquals(mana, mage.getMana(), 0.0001);
		assertEquals(2 * STARTING_BUILDING.income(0), mage.getManaIncome(), 0.0001);

		mana += mage.getManaIncome() * 10;
