...
public class GameServicePostgres implements GameService {
	...
	public Zoo buy(String name, String animal) {
		...
		int count = template.queryForObject(
				"select count from animal where username=? and animal_type=?", Integer.class, name,
		...
		if (count == 0) {
			...
			if (next != null) {
				...
				template.update("insert into animal(username, animal_type)"
						+ " select ?, ?"
						+ " where not exists (select 1 from animal where username=? and animal_type=?)"
	...
	public Zoo upgrade(String name, String animal) {
		...
		int level = template.queryForObject(
				"select level from animal where username=? and animal_type=?", Integer.class, name,
				resourcesService.index(animal));
		Building type = resourcesService.type(animal);
